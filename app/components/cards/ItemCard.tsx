...
export default function ItemCard({
    ...
    return (
        <div className={`w-full shadow-md rounded p-3 ${subject.connect && 'bg-[var(--greenBgColor)] '}`}>
            ...
            {subject?.connect && subject?.progress?.export && <div className='mt-2'>
                <b className='text-[var(--titleColor)] text-sm'>Модуль поставлен: <span className='text-[var(--mainColor)]'>{subject?.progress?.export_score} баллов</span></b>
            </div>}
        </div>
    );
}
